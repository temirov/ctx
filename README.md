...
| `--copy`              | tree, content, callchain, doc | Copy command output to the system clipboard after rendering completes. |
...
- **Optional:** `--ref` selects a branch, tag, or commit; `--rules` applies a cleanup rule set; `--doc` toggles the rendered documentation mode; `--copy` or `--copy-only` copies the rendered output (with `--copy-only` suppressing stdout). Combine these with configuration defaults as needed.
