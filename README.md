...
Set `CTX_TEST_RUN_HELPERS=1` to enable the optional helper suite, `CTX_TEST_UV` to the uv executable (defaults to `uv`), and optionally `CTX_TEST_SPM_MODEL` to an existing `tokenizer.model` file (otherwise the helper will attempt to download one). Tests automatically skip when prerequisites are missing.
